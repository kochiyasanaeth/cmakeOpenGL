...
GLuint vLoc,proLoc,tfLoc;
using msd = myShader::shaderSource; 
myShader::shaderSource sd = {
    std::pair<msd::shaderType,std::string>(msd::vertexShader,"cube100000.vert"),
...
void init(void) {
    ...
    cameraX = 0.4f,cameraY = 0.0f,cameraZ = 450.0f;
...
void display(GLFWwindow* _window,double _curTime) {
    ...
    glUseProgram(renProgram);
    vLoc = glGetUniformLocation(renProgram,"v_matrix");
    ...
    float timeFactor = (float)_curTime;
    tfLoc = glGetUniformLocation(renProgram,"tf");
...
int main()
{
    ...
    GLFWwindow* win = glfwCreateWindow(600,600,"chapter4 -program2",NULL,NULL);
