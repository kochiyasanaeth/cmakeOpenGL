...
{
    std::vector<int> ind = _sp.getIndieces();
    std::vector<glm::vec3> vert = _sp.getVertics();
    std::vector<glm::vec2> tex = _sp.getTexCoords();
    std::vector<glm::vec3> norm = _sp.getNormals();
    int numIndices = _sp.getnumIdices();
    std::cout << __FILE__<< "numIndices:" << numIndices << std::endl;
...
void display(void) {
    ...
    glDrawArrays(GL_TRIANGLES,0,_sp.getnumIdices());
}
...
int main()
{
    ...
    myScene::scene sc("chapter6 -program1",600,600);
    sc.createW();
